jQuery(document).ready( function($){
    ...
    $.fn.yit_onepage = function(){
        ...
        nav.on( 'click', 'a[href*="#"]:not([href="#"])', function(e) {
            ...
            /*
            non fa funzionare il onepage nel caso in cui la pagina non sta nella home
            if ( onepage_url != current_page_url ){
                e.preventDefault();
                window.location.href = current_page_url + this.hash;
                ...
                if( hIsSticky && ! $body.hasClass('force-sticky-header') && target.offset().top - offsetSize > header_container_height ){
    ...
    if ( $.fn.selectbox ) {
        ...
        if(wc_version < 2.4) calculate_shipping_select = 'table.shop_table.shipping p select, ';
    ...
    if ( hIsSticky ) {
        $header.imagesLoaded(function () {
            ...
            var sticky_header = $('#header-container'),
                ...
                startPoint = sticky_header.find('.header-row.row1').offset().top,
                logo = $('#logo');
            ...
            if (sticky_header.length) {
                ...
                var header_sticky_scroll = function () {
                    ...
                    if ( $window.scrollTop() + hAdminBar > startPoint ) {
                        var top = hAdminBar;
                        if ($window.width() <= 600) {
                            top = 0;
                        }

                        if ( ! sticky_header.siblings('.fixed').length ) {
                            sticky_header.clone()
                                .addClass('fixed')
                                .css({
                                    top            : -header_height,
                                ...
                                .delay(500)
                                .animate({top: top}, 500)
                                .insertAfter(sticky_header);

                            $header.removeClass('search-opened').addClass('search-closed');

                            //sticky_header.clone().addClass('header-placeholder').removeClass('fixed').insertAfter(sticky_header);
                        }

                    } else if ($window.scrollTop() + hAdminBar <= startPoint ) {

                        var fixed_header = sticky_header.siblings('.fixed');
    ...
    var add_masonry = function(){
        ...
        if ( $.fn.imagesLoaded && $.fn.masonry ) {
            $('.blog-masonry, .testimonials').each( function(){
                var container = $(this),
